...
---
  ...
  - 笔记配置文件读写函数（note_config.json）：
  {
  "source": "binance",
  "symbol": "sol"
  }
  - 笔记本实现：笔记本实际上是一个文件夹，文件夹名称即为笔记本名，配置文件存储在文件夹根目录的note_config.json中

2.笔记目录维护组件
  - 读取笔记配置文件，并维护笔记配置字段，调用笔记文件管理组件正确读写笔记配置文件
  - 读取笔记文件夹下全部笔记文件，按照文件名（日期）排序，正确的笔记文件更新逻辑，包括重命名笔记文件（更新笔记日期），更新笔记内容
...
3. k线展示组件
  - 根据笔记本配置文件和输入的时间范围，读取对应的k线数据并渲染展示
